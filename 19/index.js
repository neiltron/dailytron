...
function lineTo (x, y, offset) {
  if (offset > 0) {
    ...
    let distance = vec2.distance(pos, mousePos) / offset;

    if (distance < threshold) {
      let distanceX = threshold - (mouse.position[0] - x);
      let distanceY = threshold - (mouse.position[1] - y);
...
function bezierCurveTo (x, y, x0, y0, x1, y1, offset) {
  if (offset > 0) {
    ...
    let mousePos = vec2.fromValues(mouse.position[0], mouse.position[1]);
    let distance = vec2.distance(_pos, mousePos) / offset;
