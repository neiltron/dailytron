...
void main () {
  ...
  float turbulence = (sin(time / 800.0)) * 96.0;
  ...
  vec2 s = vec2(0);
  s.x = fbm((xy.xy + r) + (time / 10000.0 * turbulence));
  s.y = fbm(xy.xy + r + fbm(time / -s) / 100.0);

  // vec3 color = vec3(s, r.x);
  vec3 color = vec3(r.yx, s.x) * turbulence;
  ...
  pos.x *= (u_offset * 4.0 - 10.0) * (u_transformX ? color.z : 1.0) * 100.0;
  pos.y = sin(2.0 * 3.14 * angle);
  pos.z = cos(2.0 * 3.14 * angle);
