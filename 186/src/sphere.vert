...
void main () {
  ...
  vec2 r = vec2(0);
  r.x = fbm(xy / 100.0 + vec2(t / (xy.xy * -10.0)));
  ...
  pos.x -= u_offset / 20.0;
  pos.x -= u_column * 6.0 - 12.0;
  pos.z -= u_row * 6.0 - 12.0;
  // pos.x -= 4.0;
  // pos.z -= 30.0;
  // pos.y += 5.0;

  // pos = rotateZ((sin(time - u_offset / (400.0 / u_offset))) - 1.2) * pos;
  pos = rotateZ((u_mousepos.y - u_resolution.y / 2.0) / u_resolution.y / 4.0) * pos;
  pos = rotateX((u_mousepos.x - u_resolution.x / 2.0) / u_resolution.x / 4.0) * pos;
  pos = rotateY(90.0 * DEG_TO_RAD) * pos;
