...
const createFBOs = (width, height, fn) => {
  ...
  const textureData = new Float32Array(width * height * 4).fill(1).map(fn);
...
skullImg.onload = () => {
  skull = regl.texture(skullImg);
}
skullImg.src = './depth.jpg';

const faceImg = document.createElement('img');

faceImg.onload = () => {
  face = regl.texture(faceImg);
  regl.frame(render);
}

faceImg.src = './depth2.jpg';