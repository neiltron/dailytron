...
void main () {
  ...
  vec2 coords = (gl_FragCoord.xy / u_resolution.xy) * 2.0 - 1.0;
  vec2 adjusted = coords.xy * (_color.xy / 10.0 * amplitude) + .5;



  vec4 color = texture2D(u_texture, 1.0 - .5 * mix(coords, adjusted, amplitude));
