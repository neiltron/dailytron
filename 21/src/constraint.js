...
export default class Constraint {
  ...
  draw() {
    if (this.drawConstraint) {
      ctx.lineTo(this.points[0].position[0], this.points[0].position[1]);
  ...
  update() {
    ...
    let translateX = diffX * (this.stiffness + (.0011 * (Math.sin(Date.now() / 300)))) * difference;
    let translateY = diffY * (this.stiffness + (.0011 * (Math.sin(Date.now() / 250)))) * difference;
