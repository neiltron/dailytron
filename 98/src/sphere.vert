...
void main () {
  ...
  _pos /= 2.0 * sin(time / 4.0) - 2.5;

  _pos.x *= (sin(time * mod(position.x, .6)) + cos((time * mod(position.x, .5)))) / 2.0;
  _pos.y *= (cos(time * mod(position.y, .6)) + sin((time * mod(position.y, .5)))) / 2.0;
  _pos.z *= (cos(time * mod(position.z, .6)) + sin((time * mod(position.z, .5)))) / 2.0;
